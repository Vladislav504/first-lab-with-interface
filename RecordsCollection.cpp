...
void RecordsCollection::checkChronologicalOrder() const {
    for (const std::string& component : (std::string[]){"WateringRecord", "BranchRecord"}) {
        ...
        while (index < length) {
            ...
            if (prev_date[0] != 0 && prev_date[1] != 0 && prev_date[2] != 0) {
                if (prev_date[2] != year) {
                    assert(prev_date[2] < year);
                    assert(year - prev_date[2] == 1);
                } else if (prev_date[1] != month) {
                    assert(prev_date[1] < month);
                    assert(month - prev_date[1] == 1);
                    if (prev_date[2] % 4 == 0 &&
                        (prev_date[2] % 100 != 0 || prev_date[2] % 400 == 0)) {
                        if (prev_date[1] == 2) {
                            assert(prev_date[0] == 29);
                        }
                    }
                    if (prev_date[1] >= 1 && prev_date[1] <= 7) {
                        if (prev_date[1] % 2 != 0) {
                            assert(prev_date[0] == 31);
                        } else {
                            assert(prev_date[0] == 30);
                        }
                    } else {
                        if (month % 2 != 0) {
                            assert(prev_date[0] == 30);
                    ...
                    assert(prev_date[0] < day);
                    assert(day - prev_date[0] == 1);
                }
            }
            prev_date[2] = year;
            prev_date[1] = month;
...
void RecordsCollection::checkGrooming() const {
    int days = 0;
    int weekBranches = 0;
    for (int index = 0; index < length; index++) {
        days++;
        ...
        if (collection[index]->identity() == "WateringRecord") {
            assert(collection[index]->getRecordValue() >= 1 &&
